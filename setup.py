...
with open(path.join(HERE, "fixtodict", "__version__.py"), encoding="utf-8") as f:
    version = {}
    exec(f.read(), version)
...
RESOURCES = list(
    glob.iglob(path.join(HERE, "fixtodict", "resources/**/*"), recursive=True)
)
...
setup(
    ...
    package_data={"fixtodict": RESOURCES},
    python_requires=">=3.5",
    install_requires=[
        "nltk==3.4.5",
        "click",
